...
def get_model_name(cfg):
    ...
    if name in ['pose_resnet']:
        name = '{model}_{num_layers}_{NUM_JOINTS}'.format(model=name, num_layers=extra.NUM_LAYERS,NUM_JOINTS=cfg.MODEL.NUM_JOINTS)
        full_name = '{name}_{width}x{height}'.format(name=name,
        ...
        full_name = '{name}_{width}x{height}'.format(name=name,
                                                     width=cfg.MODEL.IMAGE_SIZE[0],
                                                     height=cfg.MODEL.IMAGE_SIZE[1])
    elif name in ['pose_resnetst']:
        name = '{model}_{num_layers}_{NUM_JOINTS}'.format(model=name, num_layers=extra.NUM_LAYERS,NUM_JOINTS=cfg.MODEL.NUM_JOINTS)
        full_name = '{name}_{width}x{height}'.format(name=name,
                                                     width=cfg.MODEL.IMAGE_SIZE[0],
                                                     height=cfg.MODEL.IMAGE_SIZE[1])
    elif name in ['tf_pose_resnet']:
        name = '{model}_{num_layers}_{NUM_JOINTS}'.format(model=name, num_layers=extra.NUM_LAYERS,NUM_JOINTS=cfg.MODEL.NUM_JOINTS)
        full_name = '{name}_{width}x{height}'.format(name=name,
                                                     width=cfg.MODEL.IMAGE_SIZE[0],
                                                     height=cfg.MODEL.IMAGE_SIZE[1])
    elif "mobilenet" in name:
        name = '{model}_{num_layers}_{NUM_JOINTS}'.format(model=name,  num_layers=extra.WIDTH_MULT,NUM_JOINTS=cfg.MODEL.NUM_JOINTS)
        full_name = '{name}_{width}x{height}'.format(name=name,
                                                     width=cfg.MODEL.IMAGE_SIZE[0],
                                                     height=cfg.MODEL.IMAGE_SIZE[1])
